...
class ProductState
{
     ...
     * @param $is_paid
     */
    public function __construct($product_id, $slug, $title, $description, $type, $version, $installed, $active = null, $is_paid = null, $tenweb_product = true)
    {
    ...
    public function set_active($wp_slug)
    {
        ...
        if ($this->type == "theme") {
            ...
            if (!is_multisite()) {
                ...
                $this->active = is_plugin_active_for_network($wp_slug);
            }
        }
    ...
    public function set_other_wp_info($wp_slug, $installed_product, $installed_products_wp_info)
    {
        ...
        if ($this->type === "theme") {
            ...
            if (!empty($installed_products_wp_info->response[$wp_slug])) {
                $this->repo_version = $installed_products_wp_info->response[$wp_slug]->new_version;
            } else if (!empty($installed_products_wp_info->no_update[$wp_slug])) {//only for plugins
